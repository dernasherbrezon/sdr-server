...
void assert_buffers(const float *expected, size_t expected_len, float *actual, size_t actual_len) {
...
START_TEST (test_terminated_only_after_fully_processed) {
  ...
  queue_put(buffer, sizeof(buffer) / sizeof(float), queue_obj);
...
START_TEST (test_put_take) {
  ...
  queue_put(buffer2, sizeof(buffer2) / sizeof(float), queue_obj);
