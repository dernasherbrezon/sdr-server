...
RUN apt-get update && apt-get install --no-install-recommends -y build-essential file valgrind cmake libconfig-dev libhackrf-dev pkg-config libairspy-dev librtlsdr-dev zlib1g-dev && rm -rf /var/lib/apt/lists/*